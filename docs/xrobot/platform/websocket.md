...
| 协议类型 | 地址 | 说明 |
|---------|------|------|
| **WSS** (推荐) | `wss://xrobo-io.qiniuapi.com/v1/ws2` | 安全WebSocket连接，使用SSL/TLS加密 |
| **WS** | `ws://xrobo-io.qiniuapi.com/v1/ws2` | 普通WebSocket连接，适用于内部网络 |
...
**停止播放(非打断)：**
此时`is_aborted`为false，建议设备在播放完毕后再处理`stop`消息。
```json
{
  "session_id": "session_12345",
  "type": "tts",
...
**停止播放(打断)：**
此时`is_aborted`为true，建议设备立即清理音频缓冲区，并切换状态。
```json
{
  "session_id": "session_12345",
  "type": "tts",
